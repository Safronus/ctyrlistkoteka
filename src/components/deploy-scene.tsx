 ...
 * two triggers:
 *
 *  - `global-error.tsx` — a top-level throw, usually a ChunkLoadError from a
 *    browser holding old HTML during a DEPLOY (mode "deploy") or a genuine
 *    runtime error (mode "error").
 *  - `DeployHealthGuard` — the page rendered but its stylesheet 404'd
 *    mid-deploy, so it's unstyled-but-not-crashed (always mode "deploy").
 *
 * Fully self-contained: inline styles + one <style> block, NO Tailwind and
 * NO next-intl, because it must render correctly even when the app's global
 * stylesheet is missing (that's the whole point of the health-guard path)
 * and when it replaces the entire app (the error-boundary path). Fixed to
 * the viewport so it works both as a standalone page and as an overlay on
 * top of a broken one.
 */
...
/** sessionStorage keys: throttle reloads (one per 20 s) and cap the total
