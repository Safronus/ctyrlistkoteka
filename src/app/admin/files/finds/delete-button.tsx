...
import { deleteFind } from "./delete-action";

interface Props {
  filename: string;
}

/** Two-step destructive control. First click reveals an inline
 *  confirmation strip; the actual server action only fires when the
 *  user submits the confirmed `<form>`. Native form submit (rather
 *  than a transition / fetch) lets the server action redirect the
 *  page to the listing on success without us round-tripping through
 *  client state. */
export function DeleteFindButton({ filename }: Props) {
