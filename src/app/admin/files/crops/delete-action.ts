 ...
 *  of the rename live under DATA_DIR so it stays POSIX-atomic.
 *  Hard-codes `findCrops`; the client cannot pass a scope. */
export async function deleteCrop(formData: FormData): Promise<void> {
  ...
  const resolved = await resolveDiskPath("findCrops", baseName);
  if (!resolved) {
...
export async function deleteCropsBulk(
  ...
  for (const raw of rawNames) {
    ...
    try {
      ...
      console.error("[admin/crops-delete] bulk row failed", {
        name: raw,
        message,
