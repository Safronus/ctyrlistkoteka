...
import { uploadLocationPhotos } from "./upload-action";
import {
  MAX_FILE_BYTES,
...
export function LocationPhotosUploadForm() {
  ...
  const onSubmit = () => {
    ...
    startTransition(async () => {
      ...
      for (let i = 0; i < total; i += MAX_FILES_PER_REQUEST) {
        ...
        try {
          const { results } = await uploadLocationPhotos(fd);
          ...
          setQueue((prev) => {
            ...
            batch.forEach((q, batchIdx) => {
              ...
              updated[idx] = {
                ...
                size: result.size,
                mapBaseName: result.mapBaseName,
              };
            });
            return updated;
          });
    ...
    <section className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
      <header className="mb-3 flex items-baseline justify-between gap-2">
        <h2 className="text-sm font-semibold text-gray-900">
          Nahrát reálné fotky lokalit
        </h2>
        <p className="text-xs text-gray-500">
          {"<mapa>_reálné foto<popis>.<jpg|jpeg|png|webp> • např. "}
          <code className="font-mono">Reykjavík_reálné foto.png</code> • max{" "}
          ...
          <ul className="divide-y divide-gray-200 overflow-hidden rounded-md border border-gray-200">
            {queue.map((q) => (
              ...
              >
                ...
                {q.status === "ok" && q.mapBaseName && (
                  <span
                    className="shrink-0 max-w-[30%] truncate font-mono text-emerald-700"
                    title={q.mapBaseName}
                  >
                    → {q.mapBaseName}
                  </span>
