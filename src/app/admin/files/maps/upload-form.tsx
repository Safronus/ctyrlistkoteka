...
import { uploadMaps } from "./upload-action";
import {
  MAX_FILE_BYTES,
...
export function MapsUploadForm() {
    ...
    for (const file of list) {
      ...
      if (!nameLooksAcceptable(file.name)) {
        additions.push({
          ...
          file,
          status: "rejected",
          reason: "Povolená přípona je jen .png",
        });
        continue;
      }
  ...
  const onSubmit = () => {
    ...
    startTransition(async () => {
      ...
      for (let i = 0; i < total; i += MAX_FILES_PER_REQUEST) {
        ...
        try {
          const { results } = await uploadMaps(fd);
    ...
    <section className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
      <header className="mb-3 flex items-baseline justify-between gap-2">
        ...
        <p className="text-xs text-gray-500">
          .png • zachovává metadata • max {fmtSize(MAX_FILE_BYTES)}/soubor •{" "}
          ...
          <ul className="divide-y divide-gray-200 overflow-hidden rounded-md border border-gray-200">
            {queue.map((q) => (
              ...
              >
                ...
                {q.status === "ok" && q.mapId !== undefined && (
                  <span className="shrink-0 font-mono text-emerald-700">
                    #{q.mapId.toString().padStart(5, "0")}
                  </span>
                )}
                {q.status !== "uploading" && (
                  <button
