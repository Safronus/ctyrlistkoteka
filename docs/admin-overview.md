...
| --- | --- | --- |
| **Import balíčku** (`/admin/import`) | Hromadný import jednoho **ZIP „balíčku pro web"** (originály + výřezy + mapy + `meta/LokaceStavyPoznamky.json`) najednou. Dvoufázově: analýza (nic nezapisuje) → přehled → potvrzení → staging souborů + merge LSP. Nezapisuje DB — připraví soubory pro `sync`. Idempotentní (přepis podle ID / MAP_ID, ne duplikace). **Tři druhy balíčku**, rozliší se podle `manifest.json` → klíč `typ`: (1) klasický plochý balíček nálezů/výřezů/map, (2) `lokacni-mapy` = v2 mapy, (3) `fotky-lokaci` = **reálné fotky lokalit** (od 2026-08-23). Fotkový balíček se ptá jako **první** — mapová větev dřív brala jakýkoli manifest a fotkový by spolkla s hláškou o neplatném manifestu map. Fotky se **nestagují pro sync**: převedou se sharpem na WebP (1600 px + náhled 400 px) rovnou do `generated/location-photos/` a na webu jsou hned. Join je **číslo lokace** (`00126_foto002.png` → `locations.id = 126`), lokalita mimo DB se odmítne. Přehled před potvrzením je **po lokalitách**: co balíček nese, co už na webu je (z importu i ručně nahrané) a co se přepíše; přepsané jde do koše. Popisek z manifestu se ukládá do `data/.admin/location-photos.json` a zobrazuje se pod fotkou. **Fotka se na webu objeví až do pěti minut po importu**, ne hned: `locationPhotos.ts` drží index složky v paměti s 5min platností a PM2 běží v clusteru, takže commit vyčistí index jen tomu workeru, který ho obsloužil — ostatní dojedou na expiraci. Není to chyba importu, soubor je na disku okamžitě; jen to musí admin říct, jinak to vypadá, že se import nepovedl. **Historie nahrávání** pod dropzonou vypisuje posledních 30 pokusů — `analyzed` (nahráno a zkontrolováno, ale nepotvrzeno), `committed`, `cancelled`, `failed` — s druhem balíčku, názvem souboru, velikostí a souhrnem. Bez ní se u dvoufázového chunkovaného importu nedalo poznat, jestli balíček vůbec dorazil, nebo jen nikdo nezmáčkl potvrzení. Zapisuje se append-only do `secure/import-history.jsonl` (mimo `DATA_DIR`, drží se 200 posledních), zápis nikdy neshodí import — selhání se jen zaloguje. | `src/app/admin/import/*`, `src/app/admin/api/import/{upload-chunk,analyze,commit,cancel}/route.ts`, `src/lib/admin/{importZip,importPackage,mapPackageImport,photoPackageImport,importHistory}.ts`, `src/lib/{locationPhotoPackage,locationPhotoCaptions,locationPhotos}.ts` |
| **QR** (`/admin/qr`) | **Čtyři záložky s vlastními počty** (nálezy, stránky, darování ve světě, CaSQB — ta má vlastní řádek níže). *QR nálezů*: hromadně podle čísel/intervalů (`?ids=` přijímá výběr z `/admin/files`), kód míří na `/n/<číslo>` — trvalá adresa, sken se počítá per nález. Volba hustoty (H/Q/M se skutečnými počty bodů), loga a titulku; velikost v cm s fyzicky kalibrovaným náhledem; ZIP (SVG + PNG 300 DPI + tiskový A4 arch). Seznam je sbalitelný, filtruje se podle čísla, podle poznámky (bez diakritiky) a „skrýt nenaskenované", řadí se podle čísla (Nejnovější/Nejstarší, výchozí nejnovější), má multi-select **obousměrně svázaný s polem čísel** (spec string je jediný zdroj pravdy), zobrazuje LSP poznámku a plné datum s časem, umí vynulovat skeny (per nález i hromadně) a **zrušit kód** (viz gotcha). Čísla mimo seznam se při stažení doplní (idempotentně přes PK). Nastavení + kalibrace v `data/.admin/qr-prefs.json`. *QR stránek*: původní generátor s tokenem `/go/<token>`. SVG fonty jen systémové; rasterizace **jen v prohlížeči** (viz gotcha níže). | `src/app/admin/qr/*`, `src/app/n/[id]/route.ts`, `src/app/go/[token]/route.ts`, `src/lib/admin/qr.ts` + `qrDensity.ts` + `qrPrefs.ts` |
| **CaSQB** (`/admin/qr` → záložka *CaSQB*) | QR kódy pro **Czech and Slovak Quality Board** — na **libovolnou https adresu** (typicky casqb.org), ve stylu design manuálu (https://casqb.visualbook.pro/en). Sdílí tabulku `qr_codes` s kódy stránek: `kind = 'casqb'`, cíl v `target_url`, vzhled v `style` (JSON, schéma `CasqbStyleSchema`). Kód kóduje **`/go/<token>`** (6 znaků — o jednu verzi QR menší než kódy stránek) a teprve redirect pošle na cíl. **Co telefon ukáže ve štítku po naskenování, je zakódovaná doména** — nikdy cíl, přesměrování žádný telefon dopředu nesleduje. Proto kódy chodí přes bránu na doméně CaSQB: **od 2026-09-22 běží `qr.casqb.org`** (CNAME na ctyrlistkoteka.cz, vlastní nginx server block podle `deploy/nginx-casqb-go.conf.template`, který `/<token>` proxyuje na `/go/<token>` a všechno ostatní 404, certifikát Let's Encrypt s automatickou obnovou). Zapnuto proměnnou `CASQB_QR_BASE_URL=https://qr.casqb.org` v `.env` na VPS (`lib/admin/casqbEncoded.ts`); bez ní by se kódovalo `/go` na naší doméně. Ověřeno telefonem: štítek říká qr.casqb.org a sken skončí na cíli. Vedlejší zisk — adresa je o 8 znaků kratší, takže kód klesl z **37 × 37 na 33 × 33 modulů** (při stejné velikosti tisku větší a čitelnější body). Alternativa, kdyby subdoména jednou padla: **redirect pravidlo přímo na casqb.org** (`casqb.org/q/<token>` → `ctyrlistkoteka.cz/go/<token>`, na WordPressu plugin Redirection s regexem a kódem 302); pak stačí změnit tu jedinou proměnnou. Změna platí pro nově stažené kódy; vytištěné fungují dál, `/go/<token>` na našem webu nikdy nepřestane platit s `utm_source=qr&utm_medium=casqb&utm_campaign=<slug popisku>`; vlastní `utm_*` v adrese mají přednost. Cílový web vidí jako odkazovač jen naši doménu (site-wide `strict-origin-when-cross-origin`, nastavuje ho i nginx — per-response `no-referrer` by prohrál, proto tam není). Adresa i vzhled jdou **měnit bez přetisku**, token je neměnný. **Renderer** `casqbQr.ts` je oddělený od `qr.ts` schválně (cizí brand, vlastní pravidla): tvary modulů čtverec/zaoblený/tečky/plynulý, oči čtverec/zaoblené/kruh, čtyři barvy z palety manuálu (`casqbBrand.ts`, hexy 1 : 1 z manuálu), korekce chyb vždy H. **Logo uprostřed** je oficiální vektor 1 : 1 (`CASQB_SYMBOL`, `CASQB_WORDMARK` — cesty zkopírované ze stažených SVG, zdroj a datum v hlavičce souboru), barva se **nevolí** — červené na světlém, bílé na tmavém, přesně dvě varianty, které manuál vydává; otvor se počítá z **ochranné zóny manuálu** (25 % výšky u symbolu, 40 % u základního loga) a je stropovaný na 16 % matice. Vlastní logo (SVG/PNG) se na serveru **vždy přerastruje na PNG** přes sharp — do stahovatelného SVG tak nikdy nevleze nic spustitelného; uloží se do `style.custom` jako base64 (≤ 280 kB), ne na disk. **Čitelnost rozhoduje skutečná čtečka, ne poměr kontrastu**: každý náhled i uložení projde `casqbDecodeCheck` (`casqbDecode.ts` — render, `sharp`, `jsqr` ve čtyřech velikostech od 400 px po 3 px/modul). Kombinace, kterou čtečka nepřečte nikde, se neuloží; kombinace přečtená jen v některých velikostech je **riskantní** — uložit jde („Vytvořit i tak“), ale panel to řekne a chce zkoušku na telefonu. Tvrdě se blokují jen moduly/pozadí a rámeček očí/pozadí pod 3 : 1 (`casqbStyleBlockers`); zornice a logo jen varují (`casqbStyleWarnings`). Důvod: poměr WCAG čtení nepředpovídá — červená zornice na Quality Blue je 2,5 : 1 a sedí přesně na prahu šedi dekodéru (jednou přečte, jednou ne), šedá zornice 3,4 : 1 na bílé nepřečte vůbec; rozhoduje, kam barva padne ve stupních šedi mezi prstenec a pozadí. Quality Blue je proto na moduly nepoužitelná (1,5 : 1), ale jako pozadí s tmavými moduly a červenými zornicemi projde — přesně to majitel chtěl mít povolené. Testy dekódují každou předvolbu i každý tvar, velké i na 3 px/modul — tečky mají poloměr 0,46, protože jsQR s pevným blokem prahování ztrácí menší tečky paradoxně na *velkém* renderu. **Poslední nastavení se pamatuje na serveru** (`qr-prefs.json` → `casqb`: cíl + styl, validované stejně jako při uložení), takže formulář po obnovení začíná tam, kde skončil. **Export**: SVG, PNG 2048 px, **vektorové PDF** (`svg2pdf.js`, stránka = kód + 5 mm; před generováním se hlídá, že logo při zvolené šířce nejde pod **5 mm** z manuálu — wordmark to ve 40 mm kódu nesplní) a **rastrový A4 arch** (stejný dialog jako u kódů stránek, `QrPdfButton` s `load`). **Vyřazení** kód nechá přesměrovávat (vizitka na stole žije dál), jen zmizí z aktivních; skeny po vyřazení se počítají zvlášť (`scanned_at > archived_at`, bez sloupce). **Vynulovat** smaže log skenů, **Smazat** i kód (token přestane fungovat — jen pro nevytištěné). **Statistiky** `/admin/qr/casqb/<id>`: dlaždice, horizont 7/30/90/365/vše, po dnech, denní doba, den v týdnu — bucketování v SQL v `Europe/Prague` (`casqbStats.ts`); jen z časové značky, žádné IP/UA/jazyk, a proto ani „unikátní“ (otisk = osobní údaj); CSV na `/admin/api/qr/casqb/<id>/scans` (pod `/admin/api`, 404 bez session). Grafy jsou serverové SVG s `<title>` na sloupcích. V panelu je i **návod na UTM** pro GA4/Plausible/Matomo. Audit: `casqb.*`. | `src/app/admin/qr/casqb-{panel,actions,types,pdf}.ts*`, `src/app/admin/qr/casqb/[id]/*`, `src/app/admin/api/qr/casqb/[id]/scans/route.ts`, `src/lib/admin/{casqbQr,casqbBrand,casqbTarget,casqbStats,casqbDecode,casqbEncoded,qrToken}.ts`, `src/app/go/[token]/route.ts` |
...
| Import balíčku fotek lokalit (commit) | streaming unzip, `parsePhotoName`, číslo lokace ověřené proti `locations`, `safeJoin` | přepisovaná fotka → `.trash`, `atomicWrite` WebP (web + `thumb/`), popisky atomicky do `data/.admin/location-photos.json` | `file.replace` (`scope: import-photo-package`) |
